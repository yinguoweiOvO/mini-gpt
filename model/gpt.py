...
class GPT(nn.Module):
    def __init__(self, d_model, nhead, feedforward, blocks_num, vocab_size, seq_max_len):
        super().__init__()
        ...
        self.dec_blocks=nn.ModuleList([
            nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=feedforward, batch_first=True) for _ in range(blocks_num)
        ])
